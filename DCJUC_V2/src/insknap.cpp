...
InsKnapsack::InsKnapsack(char *file, int id) : Instance(){
	int i;
	...
	//for(i=0; i<total_item; i++){
	//	printf("map %d val_rate %d val %d space %d\n", map[i], sorted[i], items_value[i], items_space[i]);
	//}
	//exit(1);
}
	
InsKnapsack::InsKnapsack(const InsKnapsack &other) : Instance(other){
	int i;
